...
const populateJourney =
  async (member) => {
    ...
    await member.populate([
      ...
      {
        path:
          "spiritualJourneyHistory.changedBy",
        select:
          "name email role",
      },
...
const ensureJourneyInitialized =
  async (member) => {
    if (!member) {
      return null;
    }
...
exports.getMemberJourney =
  async (
    ...
    try {
      ...
      member =
        await ensureJourneyInitialized(
          member
        );

      await populateJourney(
        member
      );
...
exports.getMemberJourneyHistory =
  async (
    req,
    res,
    next
      ...
      if (!stage) {
        return res
          ...
          .json({
            ...
            message:
              "La nouvelle étape est obligatoire.",
          });
      }

      if (
        !SPIRITUAL_STAGES.includes(
          stage
        )
      ) {
        return res
...
exports.getJourneyStats =
  async (
    ...
    try {
      ...
      // ==================================================
      // Elles sont comptées selon membershipType
      // sans modifier la base à ce stade.
